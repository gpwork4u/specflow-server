...
```
        ...
        │                         │                         │
 ┌──────▼──────┐         ┌───────▼───────┐         ┌───────▼───────┐
 │ Orchestrator │         │ Support Agent │         │ LLM Provider  │
 │   Worker     │         │ spec-writer   │         │ Ollama/OpenAI │
 │  (workflow)  │         │ tech-lead     │         │ Claude/Azure  │
 └──────────────┘         │ verifier      │         └───────────────┘
                          └───────────────┘
        │
  ┌─────┼──────────┬──────────────┐
...
│Worker││Worker ││ Worker  ││  Worker  │  poll queue + 呼叫 LLM
└──┬───┘└──┬───┘└───┬─────┘└────┬────┘  建立 ephemeral sandbox
   │       │        │           │
   ▼       ▼        ▼           ▼
┌──────┐┌──────┐┌──────┐┌──────────┐
│ Go   ││NestJS││ pnpm ││Playwright│  Ephemeral Sandbox
│ 1.22 ││Prisma││ Vite ││ Jest     │  per-task, 用完銷毀
│ lint ││TS    ││ TW   ││ Browser  │  完全乾淨
└──────┘└──────┘└──────┘└──────────┘
```
